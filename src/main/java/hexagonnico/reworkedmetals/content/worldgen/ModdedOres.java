...
public class ModdedOres {
	...
	public static OreConfiguration configurationEnd(Block ore, int count, float airExposure) {
		return new OreConfiguration(targetStatesEnd(ore), count, 1.0f - airExposure);
	}
	...
	public static List<OreConfiguration.TargetBlockState> targetStatesEnd(Block endOre) {
		return List.of(
			OreConfiguration.target(new BlockStateMatchTest(Blocks.END_STONE.defaultBlockState()), endOre.defaultBlockState())
		);
	}
	...
	public static List<PlacementModifier> uniformPlacement(int minCount, int maxCount, int minHeight, int maxHeight) {
		return List.of(
			CountPlacement.of(UniformInt.of(minCount, maxCount)),
			InSquarePlacement.spread(),
